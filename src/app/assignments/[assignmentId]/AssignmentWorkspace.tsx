...
export default function AssignmentWorkspace({ assignmentId }: { assignmentId: string }) {
  ...
  if (query.isPending) return <Message title="Loading assignment..." body="" />;
  if (query.error) return <Message title="Could not load assignment" body={getUserFacingErrorMessage(query.error)} />;
  if (!data) return null;
    ...
    await submit.mutateAsync({
      ...
      answers: questions
        .map((question) => {
          ...
          return { questionId, selectedOptionIndex: answers[questionId] ?? -1 };
        })
        .filter((answer) => answer.selectedOptionIndex >= 0),
    });
  }
  ...
  return (
    <main className="min-h-[calc(100vh-72px)] bg-ink-950 text-white">
      <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-8">
        <Link href="/dashboard" className="text-sm text-mist-400 hover:text-white">← Dashboard</Link>
        <div className="mt-8 rounded-[2rem] border border-ink-800 bg-ink-900/55 p-6">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-bloom-500">{completed ? "Assignment result" : "Assigned test"}</p>
          <div className="mt-3 flex items-end justify-between gap-4">
            <h1 className="text-3xl font-semibold">{completed ? "Your submitted answers" : "Complete the assignment"}</h1>
            {completed && <p className="text-3xl font-semibold text-bloom-300">{data.score ?? 0}%</p>}
          </div>
        </div>
...
function Message({ title, body }: { title: string; body: string }) {
  return <main className="min-h-[calc(100vh-72px)] bg-ink-950 px-4 py-10 text-white"><div className="mx-auto max-w-2xl rounded-[2rem] border border-ink-800 bg-ink-900/55 p-8"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-bloom-500">Assigned test</p><h1 className="mt-3 text-3xl font-semibold">{title}</h1>{body && <p className="mt-3 text-mist-400">{body}</p>}<Link href="/dashboard" className="mt-6 inline-flex rounded-xl border border-ink-700 px-5 py-3 text-sm font-semibold text-mist-300">Back to dashboard</Link></div></main>;
}